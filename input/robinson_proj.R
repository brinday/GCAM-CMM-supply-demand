...
NE_countries <- st_as_sf(NE_countries)
NE_graticules <- st_as_sf(NE_graticules)
NE_box <- st_as_sf(NE_box)

NE_countries_rob <- st_transform(NE_countries, crs = PROJ)
NE_graticules_rob <- st_transform(NE_graticules, crs = PROJ)
NE_box_rob <- st_transform(NE_box, crs = PROJ)

lbl.Y_sf <- st_as_sf(lbl.Y, coords = c("lon", "lat"), crs = 4326)  # WGS84
lbl.Y_sf_proj <- st_transform(lbl.Y_sf, crs = PROJ)

coords <- st_coordinates(lbl.Y_sf_proj)
lbl.Y.prj <- cbind(lbl.Y, X.prj = coords[, 1], Y.prj = coords[, 2])
...
# ggplot() +
#   geom_sf(data = NE_countries_rob, color = "black") +
#   geom_sf(data = NE_graticules_rob, color = "gray80") +
#   geom_text(
#     data = lbl.Y.prj,
...
# ggsave("World map Robinson projection - ggplot.png", 
